...
## QEMU CXL VM (optional -- deliberately not brought up)
...
**Status on this project's HPC box: prerequisites checked, VM deliberately
not brought up.** `bash scripts/build_qemu_cxl.sh check` ran clean (QEMU
8.2.2, `cxl-type3` device support present). The one missing prerequisite,
`/dev/kvm` read/write access, needs the account added to the `kvm` group --
a low-risk, additive, single-account permission change on its own, but
actually running the VM afterward is one more resource-consuming process on
a shared machine, for a layer that changes zero reported numbers either way.
Given that trade on a box other people are actively using, the decision was
to skip VM bring-up and rely solely on gem5 + DRAMSim3 for every CXL figure
in this project -- exactly what docs.md 4.4 already frames as the normal
case (QEMU as strengthening, not a requirement). The tooling below is left
in place, fully checked and ready, for a dedicated box or maintenance window
later.
